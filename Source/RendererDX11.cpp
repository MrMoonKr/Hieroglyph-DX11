...
//--------------------------------------------------------------------------------
bool RendererDX11::Initialize( D3D_DRIVER_TYPE   DriverType   /*= D3D_DRIVER_TYPE_HARDWARE*/ ,
    ...
    if ( DriverType == D3D_DRIVER_TYPE_HARDWARE )
    {
        for ( auto pAdapter : vAdapters )
        {
            hr = D3D11CreateDevice(
                ...
                1 ,
                D3D11_SDK_VERSION ,
                m_pDevice.GetAddressOf() ,  // 생성된 디바이스가 여기에 저장됩니다.
                &CreatedLevel ,             // 생성된 피처 레벨 ( DX 11.0, 11.1 등 ) 이 여기에 저장됩니다.
                pContext.GetAddressOf() );  // 생성된 디바이스 컨텍스트가 여기에 저장됩니다.
...
void RendererDX11::ProcessTaskQueue()
{
    ...
    else
    {
        ...
        for ( int i = m_vQueuedTasks.size() - 1 ; i >= 0 ; i -= NUM_THREADS )
        {
            ...
            for ( int j = 0 ; j < NUM_THREADS ; j++ ) // 각 스레드에 태스크 할당
            {
                if ( ( i - j ) >= 0 )
                {
                    count++;
            ...
            WaitForMultipleObjects( count , g_aEndEventHandle , true , INFINITE ); // 모든 스레드의 태스크가 끝날 때까지 대기
